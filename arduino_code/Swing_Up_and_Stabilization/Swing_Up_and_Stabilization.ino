...
const float J = m*sq(l)/3;          // moment of inertia of the pendulum
...
float EE = 0.0;                     // energy error 
float PE = 0.0;                     // position error
float E_pendulum = 0.0;             // energy of the pendulum

// INITIALIZE CONTROLLER PARAMETERS
const float dT   = 0.008; // sampling time (s)

const int KE     = 4;     // controller gain for energy error
const int KC     = 20;    // controller gain for position error

const int K1     = 32.7807; // controller gain for th   
const int K2     = 4.7813;  // controller gain for thd 
const int K3     = -2.9829; // controller gain for x    
const int K4     = -3.7504; // controller gain for xd  
    ...
    if(timer_on and controller_on){
      if( !digitalRead(LSW) && !digitalRead(RSW) ){     
          if(abs(th_k) > 0.1745){
            ...
            E_pendulum = 0.5*J*sq(thd_k) + m*g*(l/2)*(cos(th_k) + 1);
            Serial.println("E_pendulum: " + String(E_pendulum));
            ...
            if (EE*thd_k*cos(th_k) == 0){
              ...
              u_k = KE*sign(EE*thd_k*cos(th_k)) + KC*PE;
            }     
