...
class AbstractInvertedPendulum:
    def __init__(self, g, l, b, m, h):
        ...
        self.x_min = [-np.inf, -np.inf, -0.32, -np.inf]  # State lower limits
        self.x_max = [np.inf, np.inf, 0.21, np.inf]  # State upper limits
...
def controller_function(pend, x, u, stop_flag, controller_type, start_delay):
...
def pendulum_function(pend, x, u, start_delay, stop_iter, stop_flag):
...
def plotting_function(pend, x, u, stop_flag):
    ...
    while True:
        ...
        line[4], = axs[4].plot(time[:-1], u_trajectory_conc, label=label[4])
...
def main():
    ...
    if simulation:
        ...
        baud_rate = 115200
        stop_iter = 200
        pend = RealInvertedPendulum(g=9.81, l=0.41, b=0.016, m=0.3, h=1/125, port=port, baud_rate=baud_rate)
    ...
    time.sleep(2)
    p1 = Process(target=controller_function, args=(pend, x_shared, u_shared, stop_flag, controller_type, start_delay))
    p2 = Process(target=pendulum_function, args=(pend, x_shared, u_shared, start_delay, stop_iter, stop_flag))
    p3 = Process(target=plotting_function, args=(pend, x_shared, u_shared, stop_flag))
    p1.start()
    p2.start()
    p3.start()
